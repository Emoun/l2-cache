...
class OcpCacheWrapperMultiCorePort(
                           nCores: Int,
                           addrWidth: Int,
                           coreDataWidth: Int,
                           coreBurstLen: Int,
  ...
  val core = Vec(nCores, new OcpBurstSlavePort(addrWidth, coreDataWidth, coreBurstLen))
...
class OcpCacheWrapperMultiCore(
                                nCores: Int,
                                addrWidth: Int,
                                coreDataWidth: Int,
                                coreBurstLen: Int,
  ...
  val io = IO(new OcpCacheWrapperMultiCorePort(nCores, addrWidth, coreDataWidth, coreBurstLen, memDataWidth, memBurstLen))
  ...
  for (coreIdx <- 0 until nCores) {
    ...
    coresOcpAdapter(coreIdx).io.ocpBurst <> io.core(coreIdx)
