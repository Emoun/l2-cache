...
import caches.hardware.pipelined.cache.{CacheMemUpdateIO, CacheMemory, CacheUpdateEntryIO, WbFifoPushIO}
...
class Read(memSizeInBytes: Int, nCores: Int, nWays: Int, reqIdWidth: Int, tagWidth: Int, indexWidth: Int, blockOffWidth: Int, blockWidth: Int, subBlockWidth: Int)  extends Module {
  val io = IO(new Bundle {
    ...
    val update = Flipped(new CacheUpdateEntryIO(nCores = nCores, nWays = nWays, reqIdWidth = reqIdWidth, tagWidth = tagWidth, indexWidth = indexWidth, blockWidth = blockWidth, subBlockWidth = subBlockWidth))
    val stall = Input(Bool())
  })

  val dataMem = Module(new CacheMemory(memSizeInBytes, nWays, blockWidth / 8, subBlockWidth / 8))

  // TODO: Think what to do, if a request is reading out older data while a older request (miss) is trying to override the line
  //  with new data
  ...
  io.wbQueue.push := isRepDirtyReg && !isHitReg && reqValidReg
  ...
  io.update.valid := reqValidReg && (isHitReg || !repValidReg) // It is a valid response if it is a hit or a rejection // TODO: If adding rejection queue this should not be the case
