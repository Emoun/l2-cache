...
class CacheToOcpBurstSlaveAdapterTest extends AnyFlatSpec with ChiselScalatestTester {
  "OcpBurstSlaveAdapter" should "work" in {
    val addrWidth = 16
    val dataWidth = 32
    val burstLen = 4

    test(new OcpBurstSlaveToCacheAdapter(addrWidth, dataWidth, burstLen)).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      // Default Assignments
      // ---- Core Port Inputs
