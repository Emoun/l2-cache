...
class WriteBackFifoTest extends AnyFlatSpec with ChiselScalatestTester {
  "WriteBackFifo" should "push and pop entries correctly" in {
    test(new WriteBackFifo(tagWidth = 16, indexWidth = 8, blockOffsetWidth = 4, blockWidth = 32, queueDepth = 2)).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      // Initialize inputs
      ...
      dut.io.pushEntry.blockOffset.poke("h5".U)
      ...
      dut.io.pushEntry.blockOffset.poke("h8".U)
      ...
      dut.io.popEntry.blockOffset.expect("h5".U)
      ...
      dut.io.popEntry.blockOffset.expect("h8".U)
