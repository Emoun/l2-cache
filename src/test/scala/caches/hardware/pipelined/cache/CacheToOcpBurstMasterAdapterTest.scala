...
class CacheToOcpBurstMasterAdapterTest extends AnyFlatSpec with ChiselScalatestTester {
  "OcpBurstMasterAdapter" should "work" in {
    val addrWidth = 16
    val dataWidth = 32
    val burstLen = 4

    test(new CacheToOcpBurstMasterAdapter(addrWidth, dataWidth, burstLen)).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      // Default Assignments
      ...
      // ---- Memory Interface Inputs
      dut.io.ocpBurst.S.Resp.poke(ocp.OcpResp.NULL)
      ...
      dut.io.ocpBurst.M.Cmd.expect(ocp.OcpCmd.RD)
      ...
      dut.io.ocpBurst.S.Resp.poke(ocp.OcpResp.DVA)
      ...
      dut.io.ocpBurst.M.Cmd.expect(ocp.OcpCmd.IDLE)
      ...
      dut.io.ocpBurst.M.Cmd.expect(ocp.OcpCmd.WR)
